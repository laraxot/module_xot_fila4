...
# 🐄 SUPER MUCCA SCRIPT - Risolve conflitti di merge prendendo sempre "current change"
# Autore: Super Mucca AI Assistant
# Data: 2025-09-19
# Scopo: Risolvere automaticamente tutti i conflitti di merge prendendo la parte "HEAD" (current)
...
echo "🐄  Risolve conflitti prendendo current change"
echo "🐄 ==============================================="
echo -e "${NC}"

# Directory di lavoro
WORK_DIR="/var/www/_bases/base_predict_fila4_mono/laravel"
cd "$WORK_DIR"
...
resolve_file_conflicts() {
    ...
    # Usa awk per processare il file e prendere solo la parte "current" (HEAD)
    awk '
    BEGIN { 
        in_conflict = 0
        keep_current = 1
...
main() {
    ...
    # Chiedi conferma
    echo -e "${YELLOW}⚠️  ATTENZIONE: Questo script prenderà sempre la 'current change' (HEAD)${NC}"
    ...
    if ! find_conflict_files >/dev/null 2>&1; then
        ...
        echo -e "${BLUE}   3. Committa i cambiamenti: git add . && git commit -m 'Risolti conflitti merge - current change'${NC}"
...
# Funzione di aiuto
show_help() {
    echo -e "${BLUE}🐄 SUPER MUCCA MERGE CONFLICT RESOLVER - CURRENT CHANGE${NC}"
    echo ""
    ...
    echo "  (marker <<<<<<< HEAD, =======, >>>>>>> branch) e li risolve"
    echo "  automaticamente prendendo sempre la 'current change' (HEAD)."
    echo ""
    echo -e "${YELLOW}DIFFERENZA DALL'ALTRO SCRIPT:${NC}"
    echo "  • resolve_merge_conflicts_incoming.sh → prende 'incoming change'"
    echo "  • resolve_merge_conflicts_current.sh  → prende 'current change' (questo script)"
    echo ""
