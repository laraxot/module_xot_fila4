...
main() {
    ...
    if ! find_conflict_files >/dev/null 2>&1; then
        ...
        echo -e "${BLUE}   3. Committa i cambiamenti: git add . && git commit -m 'Risolti conflitti merge (current)'${NC}"
...
# Funzione di aiuto
show_help() {
    echo -e "${BLUE}🐄 SUPER MUCCA MERGE CONFLICT RESOLVER (CURRENT)${NC}"
    echo ""
    ...
    echo ""
    echo -e "${YELLOW}DIFFERENZA CON resolve_merge_conflicts_incoming.sh:${NC}"
    echo "  • incoming: prende la versione del branch remoto (develop)"
    echo "  • current:  prende la versione del branch locale (HEAD)"
    echo ""
