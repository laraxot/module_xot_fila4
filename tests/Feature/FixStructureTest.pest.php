...
test('creates necessary directories and files', function () {
    // Run the command
    artisan('xot:fix-structure')->assertExitCode(0);
...
test('handles errors gracefully', function () {
    ...
    mkdir($nonWritableDir, 0o755, true); // Ensure directory exists before changing permissions
    chmod($nonWritableDir, 0o555);

    // Run the command and expect an error
    artisan('xot:fix-structure')->assertExitCode(1);
